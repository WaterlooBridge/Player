...
typedef struct IjkIOCacheContext {
    ...
    char cache_map_path[CACHE_FILE_PATH_MAX_LEN];
    int fd;
...
static void ijkio_manager_parse_cache_info(IjkIOCacheContext *app_ctx, char *file_path) {
...
static int ijkio_cache_open(IjkURLContext *h, const char *url, int flags, IjkAVDictionary **options) {
    ...
    if (t) {
        char md5[33];
        compute_string_md5(url, strlen(url), md5);
        int len = strlen(t->value) + strlen(md5) + 6;
        snprintf(c->cache_file_path, len, "%s/%s.tmp", t->value, md5);
        snprintf(c->cache_map_path, len, "%s/%s.map", t->value, md5);

        c->cache_info_map = ijk_map_create();
        ijkio_manager_parse_cache_info(c, c->cache_map_path);
    }

    if (0 == strlen(c->cache_file_path)) {
        c->cache_file_close = 1;
    }

    c->threadpool_ctx       = c->ijkio_app_ctx->threadpool_ctx;
    c->cache_count_bytes    = &c->ijkio_app_ctx->cache_count_bytes;
    if (!c->threadpool_ctx || !c->cache_info_map) {
        return -1;
    }
    ...
    if (!c->cache_file_close) {
        do {
            ...
            if (c->tree_info == NULL) {
                ...
                if ((c->tree_info->physical_size > 200 * 1024 || c->tree_info->physical_size == c->tree_info->file_size) && c->tree_info->file_size > 0) {
...
static int ijkio_cache_resume(IjkURLContext *h) {
    ...
    if (0 == strlen(c->cache_file_path) || c->cache_file_close) {
        c->cache_file_close = 1;
    } else {
        if (c->cache_file_forwards_capacity) {
            int64_t seek_ret = lseek(c->fd, c->last_physical_pos, SEEK_SET);
...
static void ijkio_cache_destroy(IjkIOCacheContext *h)
{
    FILE *map_tree_info_fp = NULL;

    map_tree_info_fp = fopen(h->cache_map_path, "w");
    if (map_tree_info_fp) {
        ijk_map_traversal_handle(h->cache_info_map, map_tree_info_fp, ijkio_manager_save_tree_to_file);
